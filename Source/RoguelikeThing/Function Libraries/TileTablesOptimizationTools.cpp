    ...
    if (OldDimensions != CurrentDimensions) {
        ...
        if (OffsetDifference.Max.Col < 0) {
            for (int col = 0; col > OffsetDifference.Max.Col; col--) {
                UE_LOG(TileTablesOptimizationTools, Warning, TEXT("old: %s, new %s"), *OldDimensions.ToString(), *CurrentDimensions.ToString());
                for (int row = CurrentDimensions.Min.Row; row <= CurrentDimensions.Max.Row; row++) {
                    if (TilesCoordWrapper->FindWidget(row, OldDimensions.Min.Col + col - 1)) {
                        UE_LOG(TileTablesOptimizationTools, Warning, TEXT("fffffff r: %d, c: %d"), row, OldDimensions.Min.Col + col - 1);
                        continue;
                    }
                    else
                        UE_LOG(TileTablesOptimizationTools, Warning, TEXT("ddddddd r: %d, c: %d"), row, OldDimensions.Min.Col + col - 1);
                    ...
                    TilesCoordWrapper->AddWidget(row, OldDimensions.Min.Col + col - 1, Tile);

                    TilesGridPanel->AddChildToUniformGrid(Tile, row, OldDimensions.Min.Col + col - 1);
                }

                for (int row = OldDimensions.Min.Row; row <= OldDimensions.Max.Row; row++) {
                    if (!TilesCoordWrapper->RemoveWidget(row, OldDimensions.Max.Col + col)) {
                        UE_LOG(TileTablesOptimizationTools, Warning, TEXT("fffffff r: %d, c: %d"), row, OldDimensions.Max.Col + col);
                    }
                    else
                        UE_LOG(TileTablesOptimizationTools, Warning, TEXT("ddddddd r: %d, c: %d"), row, OldDimensions.Max.Col + col);
                }
            }
        }
    }
